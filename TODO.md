...
app/
  ...
  page.tsx                       HomeInner — 검색·페이지네이션·디테일 패널 통합. 헤더에 JournalEntryLink (FEATURE_JOURNAL 가드)
...
- M3 진행 중
  ...
  - PR2 ✅ — 저널 홈(/journal/[issn]) shell + IssueExplorer + /api/journal/issues
  - PR3 ✅ — 주제 탐색(TopicExplorer + /api/journal/topic) + 최근 트렌드(TrendDigest + /api/journal/trend) + 탭 분기(JournalTabs, ?tab=issue|topic|trend) + JournalPaperList 공통 추출 + lib/trend.ts (Gemini batch JSON)
  - 다음: M3 마감 검증 — FEATURE_JOURNAL=1로 prod 검증 + git push (origin/master에 GitHub raw catalog 노출되어야 fallback에서 raw로 전환). 그 후 M4 (Auth.js + Neon)

...
function trendSystemInstruction(
  ...
  if (language === "en") {
    return [
      ...
      `narrationScript: Write a natural spoken-word script (NOT bullet points) for TTS narration. Structure: brief intro → each theme explained conversationally → closing implication. Separate intro, each theme, and closing into distinct paragraphs using blank lines (two consecutive newlines, \\n\\n). Target length: ${longNarration ? "7–10 minutes" : "3–5 minutes"} when read aloud at normal pace. Tone: senior colleague briefing a busy clinician, not an academic lecture.`,
      `RULES:`,
      `- 3 to 5 themes only.`,
  ...
  return [
    ...
    `clinicalImplication: 2-3 sentences on what a busy clinician should take away from this period's literature as a whole.`,
    `narrationScript: Write a natural spoken-word script (NOT bullet points) for TTS narration. Structure: brief intro → each theme explained conversationally → closing implication. Separate intro, each theme, and closing into distinct paragraphs using blank lines (two consecutive newlines, \\n\\n) — this applies to Korean output as well. Target length: ${longNarration ? "7–10분" : "3–5분"} when read aloud at normal pace. Tone: senior colleague briefing a busy clinician, not an academic lecture. Use Korean sentence flow naturally, embed English terms where standard.`,
    `RULES:`,
    `- 3 to 5 themes only.`,

...
| 자연어 → 검색식 / 트렌드 분석 | Gemini 단일 (멀티 provider 제거 — 2026-05-16). tier 분리: fast=2.5-flash-lite, balanced=3.1-flash-lite, heavy=3-flash-preview |
| TTS — **default = Google Cloud TTS Neural2** | Naver Clova(BYOK 선택 시만 — 2026-05-16부터 격하, 월 기본료 9만 부담), Gemini TTS(fallback) |
...
- `/api/tts` — narration 생성 + provider.synthesize. default = Google Cloud TTS (구독자 강제, BYOK만 Clova 등 선택 가능)
...
- BYOK 1회 결제 (19,900): lifetime Pro+TTS 자유 라이선스. 우리 서버 Gemini 키 사용 (본인 키 강제 X). TTS provider 자유 선택 (Clova 포함)
- Balanced 월 구독 (4,900) / Pro 월 구독 (9,900): 빌링 키 기반 자동결제. 구독자는 TTS = GC 강제
- 사업자 등록·통신판매업 신고 완료 후 8단계에서 `TOSS_LIVE_MODE=1`

### v3 신규 — 관리자 회원 관리 (service-cleanup 이후)

`ADMIN_EMAILS` env 기반. 라우트는 `lib/admin.ts requireAdmin()`이 layout에서 가드 → 비관리자 `notFound()`.

- `/admin` — 사용자 목록 (정렬: 가입일/사용량/최근 활동, 검색, 페이지네이션)
- `/admin/users/[id]` — 사용자 상세 (프로필/구독/사용량/임상과 prefs/최근 audit 10건 inline) + 액션 (Plan 강제 변경/구독 해지/계정 삭제, 본인 lockout 방지)
- `/admin/audit` — admin_audit_log 뷰어 (필터·CSV export)
- `/admin/metrics` — 사용자/구독/활동 요약 + 30일 일별 가입 trend

Admin 액션 라우트(`/api/admin/users/[id]/{plan,cancel,delete}`)는 `lib/admin-audit.ts logAdminAction()`으로 자동 기록. `admin_audit_log` 테이블은 add-only, FK 없음(스냅샷 이메일 보존).
...
| v2 결정 | v3 (현행 2026-05-16) |
|---|---|
| 단일 AI 스택 (Gemini만) | **유지** (service-cleanup에서 멀티 provider 시도→철회). 모든 등급 Gemini 고정 + tier 분리(fast/balanced/heavy = 2.5-lite/3.1-lite/3-flash-preview). Claude/OpenAI/Grok 코드는 dormant 보존 |
| Auth.js / Neon / Drizzle 전부 제거 | **부활** (M4). v1.1 패턴 + v3 신규 schema (subscriptions/usage_monthly/user_*_prefs/admin_audit_log) |
| 로그인 없음 (anonymous ID만) | **로그인 필수로 강화** (Phase C-2). 검색/요약/TTS는 logged-in 전용. 트렌드 헤드라인만 logout 허용. fulltext는 비용 0이라 logout 허용 (mobile Safari 세션 이슈 회피) |
| TTS는 narration only | 유지. dialogue 모드 부활 안 함 |
| TTS provider chunk 분할 (Clova 1900, GC 4500) | 유지. 구독자는 GC 강제 (Clova는 BYOK 선택 시) |
| TTS 변환 자동재생 X, 라이브러리 append만 | 유지 (출퇴근 시나리오) |
| WAV 헤더 수동 래핑 (Gemini PCM) | Gemini fallback 시에만 사용. Clova/GC는 MP3 |
| API 키 client localStorage + X-Paperis-Keys 헤더 | 유지. BYOK 결제는 lifetime Pro+TTS 자유 라이선스(우리 서버 키 사용). 본인 키 입력은 외부 서비스(TTS/PubMed/Unpaywall) quota 분리용 선택 옵션 |
...
| 테마 = class 기반 dark + FOUC inline script + suppressHydrationWarning | 유지 |
| 검색 안전망 (PubMed JSON parse / Gemini control char sanitize) | 유지 |
| 한국어 우선 UI | 유지 + Phase 2-A/B/C로 영어 완주 (랜딩/API 분기/앱 UI i18n) |
